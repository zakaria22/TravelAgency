...
<div id="shoutout"><img src="images/joinnow_shoutout.jpg"
	alt="Join Now! It's Free" width="168" height="126" /></div>
<div id="useractions">
<div id="headings" align="right">
<h2><img src="images/create_agent_icon.jpg" alt="Agent Account"
	width="27" height="22" /> <a href="#">Espace Client</a></h2>
</div>
</div>
</div>
<div class="clear">&nbsp;</div>
...
<ul class="menu">
	...
	<ul>
		...
		<li><a href='AReservationsVol.jsp'  class="highlight">Annuler Reservations Vols</a></li>
	</ul>
	</li>
	<li><a href="#">Reservations Circuits </a>
	<ul>
		<li><a href='ReservationsCircuits.jsp'>Mes Reservations
		Circuits</a></li>
		<li><a href='AReservationsC.jsp'>Annuler Reservations
		Circuits</a></li>
	</ul>
	</li>
...
<h1>Annuler une Reservation Vol</h1>
<div id="moredetails">
...
<table border="1">
	...
	<%
		try {
			...
			GestionVol GV = (GestionVol) ctxt
					.lookup("GestionVolBean/remote");
			...
			for (int i = 0; i < reservations.size(); i++) {
				int cp = 0;
				if (reservations.get(i).getTypeReserv().equalsIgnoreCase(
						"vol")) {
					dateDepart.clear();
					Date dateD = new Date(reservations.get(i).getVol()
							.getDateDepart());
	...
	<tr bgcolor="#EBEEEE">
		<td>
		<%
			out.print(reservations.get(i).getDateReserv());
		%>
		</td>
		<td>
		<%
			out.print(reservations.get(i).getVol().getVolG()
									.getAeroportD().getVille().getNomVille());
		%>
		</td>
		<td>
		<%
			out.print(reservations.get(i).getVol()
									.getDateDepart());
		%>
		</td>
		<td>
		<%
			out.print(reservations.get(i).getVol().getVolG()
									.getAeroportA().getVille().getNomVille());
		%>
		</td>
		<td>
		<%
		...
		<%
			List<Passager> P = GV.listerPassagers();
							for (int j = 0; j < P.size(); j++) {
								if (P.get(j).getReservationsConcern()
										.getIdRserv() == reservations.get(i)
										.getIdRserv()) {
									cp++;
								}
							}
		...
		<td><a
			href="servClt?idReserv=<%=reservations.get(i).getIdRserv()%>&cpt=<%=cp%>">Annuler</a></td>
		<%
			}
					}
				}
